class UsersModel:
    ...
    def insert(self, user_name, password_hash, email, credit_card, is_admin=False):
        """Вставка новой записи"""
        cursor = self.connection.cursor()
        cursor.execute('''INSERT INTO users 
                          (user_name, password_hash, email, is_admin, credit_card) 
                          VALUES (?,?,?,?,?)''',
                       (user_name, password_hash, email, int(is_admin), credit_card))
        cursor.close()
        self.connection.commit()
...
class CarsModel:
    ...
    def insert(self, model, price, power, image_id, dealer):
        """Добавление автомобиля"""
        cursor = self.connection.cursor()
        cursor.execute('''INSERT INTO cars 
                          (model, price, power, image_id, dealer) 
                          VALUES (?,?,?,?,?)''',
                       (model, str(price), str(power), image_id, str(dealer)))
        cursor.close()
        self.connection.commit()
